...
// Prototype of CarbonLib, using the original CarbonLib code as initial analysis
...
namespace
{
#define IPC_NAME(x) [static_cast<size_t>(CarbonLib::IPCMessage::x)] = #x
...
{
	return allocator->Allocate<CarbonLib::Globals>("CarbonLib globals", *allocator, *managers);
}

SymbolType LibraryLookup(CarbonLib::Globals* globals, const char* symbolName, void** result)
{
	...
	char functionName[56] = "CarbonLib_";
...
void CarbonLib___LibraryInit(CarbonLib::Globals* globals, PPCVM::MachineState* state)
{
	if (state->r3 > 0)
	{
		...
		globals->uiChannel = new CarbonLib::UIChannel(programName);
	}
}
