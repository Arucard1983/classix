...
-(id)init
{
	...
	{
		NSLog(@"CarbonLibHead is not meant to be run directly. Please let CarbonLib launch it.");
...
-(void)createWindow
{
	...
	IPC_PARAM(windowRect, CarbonLib::Rect);
	IPC_PARAM(visible, BOOL);
...
{
	IPC_PARAM(key, uint32_t);
	IPC_PARAM(type, CarbonLib::Control::Type);
	IPC_PARAM(enabled, bool);
	IPC_PARAM(bounds, CarbonLib::Rect);
...
-(void)findWindow
{
	IPC_PARAM(point, CarbonLib::Point);
	[self expectDone];
	
	CarbonLib::WindowPartCode code;
...
-(void)dragWindow
{
	...
	IPC_PARAM(classicDragBounds, CarbonLib::Rect);
	[self expectDone];
...
-(NSRect)classicRectToXRect:(CarbonLib::Rect)rect reference:(NSRect)referenceRect
{
...
-(NSRect)classicRectToXRect:(CarbonLib::Rect)rect
{
	return [self classicRectToXRect:rect reference:screenBounds];
}

-(NSPoint)classicPointToXPoint:(CarbonLib::Point)point
{
	CGFloat x = point.h;
...
-(CarbonLib::Point)xPointToClassicPoint:(NSPoint)pt
{
	CarbonLib::Point outPoint;
